<template>
  <div>
    <div class="loading" v-show="!navGreens.length">
    ...
    <main id="site-list">
      ...
      <!-- 头部结束 -->
      <!-- 类名开始 -->
      <section class="site-list-taxon" v-show="navGreens.length">
        <h2>{{ getName }}</h2>
        <ul>
          <li>
      ...
      <section class="site-list-content">
        ...
        <!-- 菜名结束 -->
        <!-- 更多开始 -->
        <div id="add-more-button" v-show="navGreens.length">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      _getGreens () {
        // 请求的url地址
        let url = 'https://bird.ioliu.cn/v2?url=https://way.jd.com/jisuapi/byclass'
        ...
        axios.get(url, {
          ...
          if (res.status === 200) {
            // console.log(res.data.result.result.list[0].name)
            // console.log(res)
            this.navGreens = res.data.result.result.list
          }
        }).catch((error) => {
          console.log(error)
        })
